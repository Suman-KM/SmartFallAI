...
OUT_DIR = os.path.join(WORKSPACE_DIR, "preprocessing/05_motion_magnitude_features")
...
def load_session_raw(file_path, is_watch):
    ...
    with open(file_path, "r", encoding="utf-8", errors="replace") as f:
        ...
        for r in reader:
            ...
            try:
                ts = int(r[1])
                ax, ay, az = float(r[2]), float(r[3]), float(r[4])
                gx, gy, gz = float(r[5]), float(r[6]), float(r[7])
                pitch, roll, yaw = float(r[8]), float(r[9]), float(r[10])
                
                # Compute vector magnitudes
                acc_mag = np.sqrt(ax**2 + ay**2 + az**2)
                gyro_mag = np.sqrt(gx**2 + gy**2 + gz**2)
                
                feats = [ax, ay, az, gx, gy, gz, acc_mag, gyro_mag, pitch, roll, yaw]
                rows.append(feats)
...
def process_device(device_name, raw_dir, is_watch, split_dict):
    ...
    # Standardize 11 features fitted ONLY ON TRAIN
    ...
    scaler_info = {
        "method": "Motion_Magnitude_StandardScaler",
        "derived_features": {
    ...
    print(f"[{device_name}] Pipeline 05 Complete:")
...
def main():
    ...
    config = {
        "pipeline_id": "05_motion_magnitude_features",
        "description": "Motion magnitude features: 9-DoF IMU augmented with orientation-invariant accMagnitude and gyroMagnitude (11 channels) with Train-only standardization",
    ...
    report_md = f"""# Pipeline 05: Motion Magnitude Features Report
...
"""
    ...
    print("Pipeline 05 finished successfully.")
