...
OUT_DIR = os.path.join(WORKSPACE_DIR, "preprocessing/02_robust_scaling")
...
def process_device(device_name, raw_dir, is_watch, split_dict):
    ...
    # RobustScaler: median and IQR (Q75 - Q25) fitted ONLY ON TRAIN
    train_2d = X_train.reshape(-1, X_train.shape[-1])
    median = np.median(train_2d, axis=0)
    q25 = np.percentile(train_2d, 25, axis=0)
    q75 = np.percentile(train_2d, 75, axis=0)
    iqr = q75 - q25
    iqr[iqr == 0.0] = 1.0  # prevent division by zero
    
    def apply_robust_scaling(X):
        return (X - median) / iqr
        
    X_train_norm = apply_robust_scaling(X_train)
    X_val_norm = apply_robust_scaling(X_val)
    X_test_norm = apply_robust_scaling(X_test)
    ...
    print(f"[{device_name}] Pipeline 02 Complete:")
...
def main():
    ...
    config = {
        "pipeline_id": "02_robust_scaling",
        "description": "Robust scaling: Median and IQR normalization fitted on TRAIN only to mitigate sensor outlier sensitivity",
    ...
    report_md = f"""# Pipeline 02: Robust Scaling Report
...
"""
    ...
    print("Pipeline 02 finished successfully.")
